...
public class fail implements ITickable, IInventory{
	...
	{
		return this.hasCustomName() ? new TextComponentString(this.customName) : new TextComponentTranslation("container.distillation_utility");
	}
	...
	public static boolean isBurning(fail te) 
	{
	...
	{	
		...
		{
			--this.burnTime;
			DistUtil.setState(true, world, pos);
		}
		...
		else
		{
			...
			{
				ItemStack output = SmithingRecipe.getInstance().getDistilResult(inputs[0]);
				if(!output.isEmpty())
				{
					smelting = output;
					cookTime++;
	...
	private boolean canSmelt() 
	{
		...
		{
			ItemStack result = SmithingRecipe.getInstance().getDistilResult((ItemStack)this.handler.getStackInSlot(0));	
